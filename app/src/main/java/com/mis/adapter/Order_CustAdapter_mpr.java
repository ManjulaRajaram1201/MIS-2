...
public class Order_CustAdapter_mpr extends BaseAdapter implements Filterable {
	Context context;
	...
	public Order_CustAdapter_mpr(Context context, List<String> selected_ord,
			List<String> cust) {
	...
	private class ModelFilter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			if (constraint == null || constraint.length() == 0) {
				...
				if (MprOrderList.order_Details.equals("ON")) {
		...
		protected void publishResults(CharSequence constraint,
			...
			if (results.count == 0)
				...
				Order_CustAdapter_mpr.this.notifyDataSetChanged();
			}
		}
	}
