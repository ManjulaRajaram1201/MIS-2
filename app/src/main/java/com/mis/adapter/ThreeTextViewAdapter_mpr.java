...
public class ThreeTextViewAdapter_mpr extends ArrayAdapter<String> {
	Context context;
	...
	public ThreeTextViewAdapter_mpr(Context context, List<String> ordNo) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		try {
			...
			cursor = handler.getReceiptNo(orderNum.get(pos));
			handler.closeDatabase();

			if (cursor.getCount() > 0) {
				shipNo = cursor.getString(0);
				try {
					//In MprExport we had set the Recno to Ordno if some 
